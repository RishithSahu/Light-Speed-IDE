...
/// clicks are inset by the same amount.
...
pub fn geometry(
    ...
    for menu in MENUS {
        let width = menu.title.chars().count() as f32 * char_width + padding * 2.0;
        titles.push(Rect::new(x - padding, bar.y, width, bar.height));
        x += width;
    }
    ...
    let (dropdown, items) = match state.open.and_then(|index| Some((index, *titles.get(index)?))) {
        Some((index, title)) => {
            ...
            let height = menu.items.len() as f32 * row_height + inset * 2.0;
            let panel = Rect::new(title.x, bar.bottom(), width, height);

            let mut rects = Vec::with_capacity(menu.items.len());
            for row in 0..menu.items.len() {
                rects.push(Rect::new(
                    panel.x,
...
pub fn hit(geometry: &MenuGeometry, state: MenuState, x: f32, y: f32) -> MenuHit {
    ...
    match geometry.items.iter().position(|rect| rect.contains(x, y)) {
        Some(row) => match items.get(row).and_then(|item| item.command) {
...
mod tests {
    ...
    fn the_three_required_menus_exist_with_their_required_items() {
        ...
        assert_eq!(file, vec!["New", "Open...", "Save", "Save As...", "Exit"]);

        let edit: Vec<&str> = EDIT_ITEMS.iter().map(|item| item.label).collect();
        assert_eq!(edit, vec!["Undo", "Redo", "Cut", "Copy", "Paste", "Select All", "Delete"]);
    }
    ...
    fn clicking_an_item_reports_its_command() {
        ...
        let save_item = geometry.items[2];
        assert_eq!(
